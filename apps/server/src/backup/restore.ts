...
async function withRestoreLock<T>(key: string, work: () => Promise<T>): Promise<T> { const previous = restoreLocks.get(key) ?? Promise.resolve(); let release!: () => void; const current = new Promise<void>((resolvePromise) => { release = resolvePromise; }); const tail = previous.then(() => current); restoreLocks.set(key, tail); await previous; try { return await work(); } finally { release(); if (restoreLocks.get(key) === tail) restoreLocks.delete(key); } }
