...
每条记录包含必填标题与正文，并可选插入多张图片、附加一首 MP3。应用会读取歌曲文件中的标题、歌手、专辑、年份与封面，也可以继续在线识别或由用户手动修正。
...
- 同一天可发布多条记录，按北京时间精确到分钟。
- 标题仅用于编辑、搜索和索引，不会出现在阅读正文首行。
- Markdown 正文编辑与预览。
- 在源码编辑和渲染预览之间切换时，会保留当前阅读进度，不会跳回顶部。
...
- 浅色、深色和跟随系统三种主题状态，按钮循环切换。
- 已发布日记可以修改，但原始发布时间不会改变。
- 编辑保存后会返回连续时间线，并重新定位到刚刚修改的记录。
...
可使用编辑器右上角的模式按钮在源码编辑和渲染预览之间切换。无论新建还是编辑已有记录，切换前后的内容都会保持在相同阅读位置。
...
专辑封面会在音乐卡片中居中完整对齐，不会受到正文图片间距影响而偏移或被截断。
...
- 保存修改不会改变原始发布时间，并会返回时间线定位到刚刚修改的记录。
